...
export default async function BlogPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white relative overflow-hidden">
      {/* Background Vignettes matching BlogsSection & BlogsPage */}
      ...
      <div className="absolute top-0 -right-48 w-1/4 h-full bg-[#5CB338]/15 blur-[140px] rounded-full z-0 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-[#9ef01a]/5 blur-[100px] rounded-full -ml-48 -mb-48 z-0 pointer-events-none" />
      ...
      <article className="relative z-20 max-w-5xl mx-auto px-6 py-12 md:py-20">
        ...
        <header className="mb-16">
          ...
          <div className="inline-flex flex-wrap items-center gap-4 md:gap-6 px-6 py-3 rounded-full bg-white/40 backdrop-blur-md border border-black/5 shadow-sm font-body text-sm md:text-base text-black/60">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-[#5CB338]/10 flex items-center justify-center">
                <span className="text-[#5CB338] text-xs font-bold">{blog.author[0]}</span>
              </div>
              <span className="font-bold text-black">{blog.author}</span>
            </div>
            <span className="w-1.5 h-1.5 rounded-full bg-black/10 hidden sm:block" />
            <span>{blog.date}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-black/10 hidden sm:block" />
            <span className="text-[#5CB338] font-medium">{blog.readTime}</span>
          </div>
        </header>

        {/* Cover Image */}
        <div className="relative w-full aspect-[16/9] md:aspect-[2.4/1] rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden mb-20 shadow-2xl shadow-black/5 border border-black/5 bg-gray-100">
          <Image 
            src={blog.image} 
            alt={blog.title}
            fill
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_250px] gap-16">
          ...
          <div className="font-body text-lg md:text-xl text-black/80 leading-[1.7] space-y-10 max-w-3xl">
            ...
            {/* Tags display under content */}
            <div className="pt-8 border-t border-black/5 flex flex-wrap gap-2.5">
              {blog.tags.map((tag) => (
                <span 
                  key={tag} 
                  className="px-4 py-1.5 text-xs rounded-full bg-gray-50 border border-black/5 text-black/60 font-body"
                >
                  #{tag.toLowerCase()}
                </span>
              ))}
